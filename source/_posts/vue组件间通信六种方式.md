...
layout: posts
title: vue组件间通信六种方式
date: 2019-05-19 08:58:52
categories: 学习笔记
tags: [javscript, vue, 组件通信]
copyright: ture
---
...
针对不同的使用场景，如何选择行之有效的通信方式？这是我们所要探讨的主题。总结了vue组件间通信的几种方式，如props、$emit/$on、vuex、$parent / $children、$attrs/$listeners和provide/inject，
...
```
复制代码总结：父组件通过props向下传递数据给子组件。注：组件中的数据共有三种形式：data、props、computed
...
```
复制代码总结：子组件通过events给父组件发送消息，实际上就是子组件把自己的数据发送到父组件。
...
# vuex
![img](/static/images/vuex20190519092241.png)
...
![img](/static/images/atrs_20190519094042.png)
...
![img](/static/images/provide_20190519095540.png)
